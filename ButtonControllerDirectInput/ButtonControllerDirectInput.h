...
#define BUTTONDI_NO_NEW_DATA           0ULL

// Helper macro to check for errors
#define IS_BUTTONDI_ERROR(x) ((x) & BUTTONDI_ERROR_BIT)

__declspec(dllexport) int GetDirectInputDeviceList(char* buffer, int bufferSize);
...
__declspec(dllexport) const char* FindJoystickByProductStringAndUsage(
    const char* name,
    unsigned short usagePage,
    unsigned short usage);

__declspec(dllexport) void* OpenJoystickByInstanceGUID(const char* instanceGUID);
