...
int main() {
        ...
        if (GetHIDDeviceList(buffer, BUFFER_SIZE) == 0) {
                try {
                        json devices = json::parse(buffer);
                        std::cout << "Found " << devices.size() << " HID
devices.\n";

                        // Find and store the exact product string from the
device list std::string exactProductString; for (const auto& device : devices) {
                                std::string product =
device["product"].get<std::string>(); if (product.find("Kinesis") !=
...
<< "Product string: \"" << product << "\"\n"; printStringAsHex(product.c_str());
                        ...
                        if (!exactProductString.empty()) {
                                std::cout << "\nTesting with exact product
string from device list...\n"; int joystickId =
FindJoystickByProductString(exactProductString.c_str()); std::cout << "Result: "
<< joystickId << std::endl;

                                std::cout << "\nTesting with hardcoded
string...\n"; const char* hardcodedName = "Kinesis JoyStick Controller";
                                std::cout << "Hardcoded string: \"" <<
hardcodedName << "\"\n"; printStringAsHex(hardcodedName); joystickId =
FindJoystickByProductString(hardcodedName); std::cout << "Result: " <<
joystickId << std::endl;

                                // Try with vendor and product ID from the
device list for (const auto& device : devices) { if
(device["product"].get<std::string>() == exactProductString) { std::string
vendorIdStr = device["vendorID"].get<std::string>(); std::string productIdStr =
device["productID"].get<std::string>();

                                                // Convert hex strings to
integers unsigned short vendorId = std::stoi(vendorIdStr.substr(2), nullptr,
16); unsigned short productId = std::stoi(productIdStr.substr(2), nullptr, 16);

                                                std::cout << "\nTrying with
Vendor ID: 0x" << std::hex << vendorId
                                                                  << " Product
ID: 0x" << productId << std::dec << "\n"; joystickId =
FindJoystickByVendorAndProductID(vendorId, productId); std::cout << "Result: "
<< joystickId << std::endl;
                                        }
                                }
                        }
                        ...
                        std::cout << "Failed to parse device list: " << e.what()
<< std::endl; return 1;
                }
        }
        else {
